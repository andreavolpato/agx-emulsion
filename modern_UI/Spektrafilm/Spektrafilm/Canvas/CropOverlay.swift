...
struct CropOverlay: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            if size.width > 1, v.image.width > 1 {
                let corners = g.corners(in: size).map { view($0, v) }
                ZStack {
                    outline(corners)
                    thirds(g, size, v)
                    grips(g, size, v)
    ...
    /// Source-normalised → view points. The viewport is expressed against the
    /// whole frame while the crop tool is up (`Renderer.logicalSize(forSource:)`),
    /// which is exactly why the overlay can use it directly.
    private func view(_ n: CGPoint, _ v: ViewportState) -> CGPoint {
        CGPoint(x: v.offset.x + n.x * v.image.width * v.scale,
                y: v.offset.y + n.y * v.image.height * v.scale)
    }
    ...
    /// The rule-of-thirds grid, rotated with the crop. Every crop tool shows
    /// one and it is the only reason to draw inside the rectangle at all.
    private func thirds(_ g: Geometry, _ size: CGSize, _ v: ViewportState) -> some View {
        Path { p in
            for t in [1.0 / 3, 2.0 / 3] {
                let a = view(g.rotated(CGPoint(x: g.crop.x + g.crop.width * t, y: g.crop.y), in: size), v)
                let b = view(g.rotated(CGPoint(x: g.crop.x + g.crop.width * t, y: g.crop.y + g.crop.height), in: size), v)
                p.move(to: a); p.addLine(to: b)
                let c = view(g.rotated(CGPoint(x: g.crop.x, y: g.crop.y + g.crop.height * t), in: size), v)
                let d = view(g.rotated(CGPoint(x: g.crop.x + g.crop.width, y: g.crop.y + g.crop.height * t), in: size), v)
    ...
    /// Corner brackets and edge bars, at the rotated handle positions and
    /// rotated with the crop so a straightened frame's grips lie along its
    /// own edges rather than the window's.
    ...
    @ViewBuilder
    private func straightenLine(_ v: ViewportState) -> some View {
        if let line = session.straightenPreview {
            Path { p in
                p.move(to: view(line.from, v))
                p.addLine(to: view(line.to, v))
            }
